...
- **Write Tasks**: `Frends.Qconn.ModbusTcp.Write.WriteSingleCoil`, `WriteSingleRegister`, `WriteMultiple`, `ReadWriteMultiple`, and `WriteBatch`. All writes are gated by the `ModbusWritesAllowed` Environment Variable (see SECURITY.md); setting it to `false` blocks every write at Task entry before any TCP socket is opened.
